...
def image_callback(msg):
    ...
    annotated_frame = results[0].plot(labels=False,boxes=True)

    # Display the annotated frame in a named OpenCV window
    cv2.imshow("Right Camera", annotated_frame)

    cv2.waitKey(1)

if __name__ == '__main__':
    rospy.init_node('Realsense_detectR_viewer')
    ...
    rospy.Subscriber('/realsense_tertiary/color/image_raw', Image, image_callback)
