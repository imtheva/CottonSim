...
def image_callback(pCam):
    ...
    # pubminx = rospy.Publisher('mask/sky/minx',Int16,queue_size=10)
    pubminy = rospy.Publisher('mask/sky/miny',Int16,queue_size=10)
    pubmaxx = rospy.Publisher('mask/sky/maxx',Int16,queue_size=10)
    pubmaxy = rospy.Publisher('mask/sky/maxy',Int16,queue_size=10)

    pubsMaskLCenter = rospy.Publisher('mask/sky/sMaskLCenter',Int16,queue_size=10)
    pubsMaskHCenter = rospy.Publisher('mask/sky/sMaskHCenter',Int16,queue_size=10)
    pubrespxmin = rospy.Publisher('mask/sky/respminx',Int16,queue_size=10)
    pubrespymax = rospy.Publisher('mask/sky/respmaxy',Int16,queue_size=10)




    resultss= model.predict(source=pCam_cv_image,show=False,classes=[2],show_labels=False,boxes=False, retina_masks=True)



    # Ground Layer based mask points
    maskss = resultss[0].masks
    pointss = maskss[0].xy


    flattened_data = [point for sublist in pointss for point in sublist]
    ...
    sMaskLCenter = ((min(resp_xmin_points)+max(resp_xmin_points))/2)
            # print("gMaskLCenter: ",sMaskLCenter)
    sMaskHCenter = ((min(resp_xmax_points)+max(resp_xmax_points))/2)

    # pubminx.publish(int(min_x))
    pubmaxx.publish(int(max_x))
    pubminy.publish(int(min_y))
    pubmaxy.publish(int(max_y))
    pubrespxmin.publish(int(min(resp_xmin_points)))
    pubrespymax.publish(int(max(resp_ymax_points)))

    pubsMaskLCenter.publish(int(sMaskLCenter))
    pubsMaskHCenter.publish(int(sMaskHCenter))


if __name__ == '__main__':
    rospy.init_node('SkYMASKPublisher')
